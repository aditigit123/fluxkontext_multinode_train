...
class QwenImageTrainingModule(DiffusionTrainingModule):
    ...
    def forward_preprocess(self, data):
        # Check if data is batched (list) or single sample
        # Batched data has lists as values: {'image': [img1, img2, ...], 'prompt': [p1, p2, ...]}
        # Single data has single values: {'image': img, 'prompt': p}
        
        first_key = list(data.keys())[0]
        is_batched = isinstance(data[first_key], list)
        
        if is_batched:
            # Batched input: process each sample and combine
            batch_size = len(data[first_key])
            batch_results = []
            
            for i in range(batch_size):
                # Extract single sample from batch
                single_data = {key: data[key][i] for key in data.keys()}
                # Process single sample
                single_result = self._forward_preprocess_single(single_data)
            ...
            for key in batch_results[0].keys():
                if isinstance(batch_results[0][key], torch.Tensor):
                    # Stack tensors: [tensor1, tensor2, ...] -> batched_tensor
                    combined[key] = torch.cat([r[key] for r in batch_results], dim=0)
                elif isinstance(batch_results[0][key], list):
                    # Concatenate lists
                    combined[key] = [item for r in batch_results for item in r[key]]
                else:
                    # For scalars, take first (they should be same)
                    combined[key] = batch_results[0][key]
            return combined
        else:
            # Single sample: process normally (backward compatible)
            return self._forward_preprocess_single(data)
    
    def _forward_preprocess_single(self, data):
