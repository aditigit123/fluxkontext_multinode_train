...
class FluxImageUnit_InputImageEmbedder(PipelineUnit):
    ...
    def process(self, pipe: FluxImagePipeline, input_image, noise, tiled, tile_size, tile_stride):
        ...
        if isinstance(input_image, list):
            # Batched: process each image and stack
            batch_size = len(input_image)
            images = [pipe.preprocess_image(img).to(device=pipe.device, dtype=pipe.torch_dtype) for img in input_image]
            image = torch.cat(images, dim=0)  # Stack along batch dimension
            # Expand noise to match batch size
...
class FluxImageUnit_Kontext(PipelineUnit):
    ...
    def process(self, pipe: FluxImagePipeline, kontext_images, tiled, tile_size, tile_stride):
        ...
        # Check if batched: [[img1], [img2]] vs single: [img1, img2]
        is_batched = isinstance(kontext_images[0], list) if len(kontext_images) > 0 else False
        
        if is_batched:
            # Batched processing: [[img1_ctx1, ...], [img2_ctx1, ...], ...]
            batch_latents = []
            batch_image_ids = []
            for batch_kontext in kontext_images:
                # Process each batch item's kontext images
                kontext_latents = []
                kontext_image_ids = []
                for kontext_image in batch_kontext:
                ...
                # Concat kontext images for this batch item
                kontext_latents = torch.concat(kontext_latents, dim=1)
                kontext_image_ids = torch.concat(kontext_image_ids, dim=-2)
                batch_latents.append(kontext_latents)
                batch_image_ids.append(kontext_image_ids)
            # Stack batches
            kontext_latents = torch.cat(batch_latents, dim=0)
            kontext_image_ids = torch.cat(batch_image_ids, dim=0)
        else:
            # Single sample with potentially multiple kontext images
            kontext_latents = []
            kontext_image_ids = []
