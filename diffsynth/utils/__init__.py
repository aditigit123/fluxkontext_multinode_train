    ...
    def download_if_necessary(self, use_usp=False):
        if self.path is None:
            ...
            if self.local_model_path is None:
                # Use HuggingFace cache if HF_HOME is set, otherwise use ./models
                hf_home = os.environ.get('HF_HOME', None)
                if hf_home:
                    self.local_model_path = os.path.join(hf_home, "hub")
                else:
                    self.local_model_path = "./models"
            if not skip_download:
                downloaded_files = glob.glob(self.origin_file_pattern, root_dir=os.path.join(self.local_model_path, self.model_id))
                # HuggingFace uses different parameter names than ModelScope
                snapshot_download(
                    self.model_id,
                    local_dir=os.path.join(self.local_model_path, self.model_id),
                    allow_patterns=allow_file_pattern,  # HuggingFace: allow_patterns (not allow_file_pattern)
                    ignore_patterns=downloaded_files,   # HuggingFace: ignore_patterns (not ignore_file_pattern)
